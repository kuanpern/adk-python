...
from pymongo.errors import ConnectionFailure, InvalidURI, DuplicateKeyError # Specific MongoDB errors
...
class DatabaseSessionService(BaseSessionService):
  ...
  def __init__(self, db_url: str, **kwargs: Any):
    ...
    try:
      ...
      raise ValueError(f"Invalid MongoDB connection URI format for '{db_url}': {e}") from e
      ...
      raise ValueError(f"An unexpected error occurred initializing MongoDB client for '{db_url}': {e}") from e
    ...
    self.events_collection.create_index(
        [("session_id", 1), ("timestamp", 1)] # For sorting by timestamp in get_session
    )
    # An index on `id` (which is _id for events documents) is automatically present.

    self.local_timezone = get_localzone()
    logger.info(f"MongoDB session service initialized. Connected to DB: {self.db.name} at {db_url}")
  ...
  async def create_session(
    ...
    try:
      ...
      raise ValueError(f"Session with id '{session_id}' for user '{user_id}' in app '{app_name}' already exists.") from e
    ...
    last_update_time_ts = session_document["update_time"].replace(tzinfo=timezone.utc).timestamp()
  ...
  async def get_session(
    ...
    if config and config.after_timestamp:
      ...
      event_query_filter["timestamp"] = {"$gte": datetime.fromtimestamp(config.after_timestamp, tz=timezone.utc)}

    # Fetch events, sorted by timestamp (ascending for chronological order)
    event_cursor = self.events_collection.find(event_query_filter).sort("timestamp", 1)
    ...
    if config and config.num_recent_events:
      ...
      event_cursor = self.events_collection.find(event_query_filter).sort("timestamp", -1).limit(config.num_recent_events)
  ...
  async def list_sessions(
    ...
    for session_doc in results_cursor:
      # Convert update_time (UTC datetime) to Unix timestamp
      last_update_time_ts = session_doc["update_time"].replace(tzinfo=timezone.utc).timestamp()
  ...
  async def append_event(self, session: Session, event: Event) -> Event:
    ...
    session_doc_id = {"app_name": session.app_name, "user_id": session.user_id, "id": session.id}
    ...
    storage_session_update_time_ts = storage_session_doc["update_time"].replace(tzinfo=timezone.utc).timestamp()
    ...
    storage_app_state_doc = self.app_states_collection.find_one({"_id": session.app_name})
    storage_user_state_doc = self.user_states_collection.find_one(
    ...
    if session_state_delta:
      ...
      self.sessions_collection.update_one(
          session_doc_id,
          {"$set": {"update_time": update_time_for_db}}
      )
    ...
    session.last_update_time = update_time_for_db.replace(tzinfo=timezone.utc).timestamp()
  ...
  def _from_event(self, session: Session, event: Event) -> dict[str, Any]:
    ...
    if event.grounding_metadata:
      doc["grounding_metadata"] = event.grounding_metadata.model_dump(exclude_none=True, mode="json")
  ...
  def _to_event(self, doc: dict[str, Any]) -> Event:
    ...
    timestamp_from_db = doc["timestamp"].replace(tzinfo=timezone.utc).timestamp() if isinstance(doc["timestamp"], datetime) else doc["timestamp"]
    ...
    return Event(
        ...
        grounding_metadata=_session_util.decode_grounding_metadata(doc.get("grounding_metadata")),
    )

# Helper functions that operate on dictionaries remain unchanged
def _extract_state_delta(state: Optional[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
  app_state_delta = {}
  user_state_delta = {}
  session_state_delta = {}
  if state:
...
def _merge_state(app_state: dict[str, Any], user_state: dict[str, Any], session_state: dict[str, Any]) -> dict[str, Any]:
